...
def plot_time_comparison(x_measure: str, y_measure: str, time_averages: dict[str, dict[str, pd.DataFrame]],
    ...
    for group in time_averages:
        y_plot = time_averages[group][y_measure].iloc[0][2:]
        x_plot = np.arange(len(y_plot))
        ax.scatter(x_plot, y_plot, s=plot_settings.marker_size, c=plot_settings.group_colors[group], label=group)
...
def plot_cmeasure_comparison(x_measure: str, y_measure: str, cmeasure_averages: dict[str, pd.DataFrame],
    ...
    if plot_settings.group_error_bars:
        for group in cmeasure_averages:
            ...
            ax.errorbar(x_plot, y_plot, yerr=y_error, xerr=x_error, fmt='none',
                        ecolor=plot_settings.group_colors[group], errorevery=plot_settings.n_between_error,
                        elinewidth=plot_settings.error_width, alpha=plot_settings.alpha)
    # then do the scatter plots for all groups:
    for group in cmeasure_averages:
        y_plot = cmeasure_averages[group][f'{y_measure} mean']
        x_plot = cmeasure_averages[group][f'{x_measure} mean']
    ...
    if plot_settings.save_group__comparison_figures:
        ...
        os.makedirs(path, exist_ok=True)
        fig_path = generate_fig_title(path, x_measure, y_measure, plot_settings.group_model_fit,
                                      plot_settings.group_error_bars, plot_settings.fig_extension)
        fig.savefig(fname=fig_path, bbox_inches='tight')
    plt.close(fig=fig)
    return
