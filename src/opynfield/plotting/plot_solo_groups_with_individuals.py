...
def generate_view_title(path: str, x_measure: str, y_measure: str, individual_models: bool, group_model: bool,
                        group_error: bool, extension: str):
...
def plot_group_individual_comparison_cmeasure(group: str, x_measure: str, y_measure: str, individuals_x: pd.DataFrame,
                                              individuals_y: pd.DataFrame, individuals_params: pd.DataFrame,
                                              group_ys_df: pd.DataFrame, group_params: pd.DataFrame,
                                              model_spec: ModelSpecification, plot_settings: PlotSettings,
    ...
    for i in range(individuals_y.shape[0]):
        ...
        ax.scatter(x_plot, y_plot, s=plot_settings.marker_size, c=plot_settings.marker_color, alpha=0.1)
    ...
    if plot_settings.group_error_bars:
        ax.errorbar(x_plot_group, y_plot_group, yerr=y_err_group, xerr=x_err_group, fmt='none',
                    ecolor='k', errorevery=plot_settings.n_between_error, elinewidth=plot_settings.error_width,
    ...
    if plot_settings.group_model_fit:
        y_fit_group = model_spec.model.model_function(x_plot_group, *group_params.values)
    ...
    if plot_settings.save_combined_view_figures:
        path = user_inputs.result_path + '/Groups/' + user_inputs.groups_to_paths[group] + \
               '/Plots/individual_view_by_' + x_measure + '/'
        os.makedirs(path, exist_ok=True)
        fig_path = generate_view_title(path, x_measure, y_measure, plot_settings.individual_model_fit,
                                       plot_settings.group_model_fit, plot_settings.group_error_bars,
...
def plot_group_individual_comparison_time(group: str, x_measure: str, y_measure: str, individuals_y: pd.DataFrame,
                                          individuals_params: pd.DataFrame, group_y: pd.DataFrame,
                                          group_params: pd.DataFrame, model_spec: ModelSpecification,
    ...
    if plot_settings.group_error_bars:
        ax.errorbar(x_plot_group, y_plot_group, yerr=y_err_group, xerr=None, fmt='none',
                    ecolor='k', errorevery=plot_settings.n_between_error, elinewidth=plot_settings.error_width,
                    alpha=plot_settings.alpha)
    # then plot the group averages
    ax.scatter(x_plot_group, y_plot_group, s=plot_settings.marker_size, c='k')
...
def plot_components_of_solo_groups(individuals: dict[str, dict[str, pd.DataFrame]],
                                   individual_fits: dict[str, dict[str, dict[str, pd.DataFrame]]],
                                   groups: dict[str, dict], group_fits: dict[str, dict[str, dict[str, pd.DataFrame]]],
                                   model_specs: dict[str, dict[str, ModelSpecification]], defaults: Defaults,
                                   plot_settings: PlotSettings, user_inputs: UserInput):
    ...
    for group in groups['time']:
        ...
        for y_measure in groups['time'][group]:
            if y_measure != 'r':
                ...
                plot_group_individual_comparison_time(group, 'time', y_measure,
        ...
        for x_measure in ['coverage', 'pica', 'pgca', 'percent_coverage']:
            ...
            for y_measure in defaults.coverage_averaged_measures:
                plot_group_individual_comparison_cmeasure(group, x_measure, y_measure,
                                                          individuals[group][x_measure], individuals[group][y_measure],
